...
export async function composeTrack(args: {
  ...
  const { topic, concepts, priorKnowledge, targetMastery, budgetMinutes, onTrace = () => {} } =
    args;
  ...
  const result = await generateText({
    ...
    prompt: buildPrompt({ topic, conceptViews, priorKnowledge, targetMastery, budgetMinutes }),
  });
...
function buildPrompt(args: {
  ...
  const { topic, conceptViews, priorKnowledge, targetMastery, budgetMinutes } = args;
