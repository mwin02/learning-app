...
const SpineSchema = z.object({
  concepts: z.array(
    z.object({
      ...
      // The single orientation on-ramp concept (Concept.isOnRamp). Default false so
      // an author that omits it doesn't break parsing; validateSpine normalizes to
      // exactly one (the marked one, else the unique root) — see cycle.ts.
      isOnRamp: z.boolean().default(false),
    }),
  ),
  edges: z.array(
    z.object({
