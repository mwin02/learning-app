...
Features are planned in a dedicated planning conversation — the `/plan-feature` skill — which ends in a plan doc at `docs/plans/<feature>.md` with per-block briefs (blocks ≤300 LOC), then implemented block-by-block via the `/orchestrate-feature` skill, which spawns a `block-implementer` then a `block-verifier` per block, and one `block-reviewer` across the finished stack. Every block brief ends with an **Acceptance criteria** section: concrete, checkable assertions written at planning time (they drive the verifier; criteria written after implementation get bent to fit the code). Doc tiers, and where a shipped plan goes afterwards: [docs/README.md](docs/README.md) and [docs/plans/README.md](docs/plans/README.md). Two rules bind every conversation:
