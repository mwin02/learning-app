...
- **What belongs in the library is a locked standard** (`resource-standard.md`) — six clauses, a one-sided error budget (doubt admits; a weak signal never decides alone; every exclusion names its clause), and interactive resources excluded as a class. The standard carries no mechanics on purpose; how it is enforced — the three classifiers, which seam may write, and the facts that read the opposite of how they behave — is `library-enforcement.md`.
...
### Shipped — resource serveability (S1–S9 + C1–C2, PRs [#350](https://github.com/mwin02/learning-app/pull/350)–[#360](https://github.com/mwin02/learning-app/pull/360), 2026-08-18)

Enforces `resource-standard.md`, which until now was a decision with **no implementing code**: the open-web ingestion path had no content check of any kind, and clause 6 (*accurately described*) had zero rules anywhere. Plan archived at [plans/archive/resource-serveability.md](plans/archive/resource-serveability.md); mechanics in `library-enforcement.md`.

- **Three classifiers** — row-level (clauses 3/5, pure), probe-level (clauses 4/5, needs the page), metadata integrity (clause 6, returns findings each carrying its repair). Split deliberately: serveability and clause 6 have opposite remedies, exclusion vs. correction.
- **Both ingestion seams guarded** — the discovery root was previously unchecked, which was the least-trusted admission path. Clause 5 enforces on creation; clause 3 does not, because at creation `atomic` is a default rather than a judgement.
- **`type` became correctable** on an atomic row (`article | video`), and `durationMin` nullable — the repair path clause 6 needs, and the reason a mislabelled-but-good row is corrected instead of deprecated.
- **The review rubric grew from five items to seven** and learned to repair metadata rather than reject it.
- **Measured**: 108 rows fail a serveability clause (93 pickable, 22 attached); 186 fail clause 6. Three drivers — a read-only report, the sweep (deprecate / re-type / decompose, dry-run by default), and the Khan `/pt/` repoint.
- **Remaining, not blocked:** the two `--apply` runs behind their dry-run gates; a review campaign over 173 title-mismatch rows; one duplicate row and one parked `/pt/` row by hand; giving clause-3-on-edX a classifier home rather than a driver-local URL predicate.
