...
### ⭐ NEXT UP — Phase 2.5h: Content agent (exercises)

`lib/agents/content/` generates `Exercise` records (text/MCQ first) for gap-prone Lessons, fanning out in parallel during track building under a per-build cost ceiling. It's the first content a learner can _act on_ rather than just read — and the missing half of the 2.6 lesson view (the exercise slot is already scaffolded in `LessonView`, and the playground/landing-page exit criteria both call for revealing exercises). See the [Phase 2.5 block sequence](#block-sequence-each-300-loc-one-pr-per-block).

**Then:** **2.5i–k** (delivery-mode classifier → notebooks → playground map/Track) → **2.6a** (landing page) → 2.75 (Programs) → 3 (auth + Stripe) → 4–6.
