...
- [x] **2.5f — Async thickener + spine-hole remediation.** The standing map-builder worker. Three responsibilities: (1) add frontier concepts + extra candidate resources (the alternates) after the spine; (2) **remediate spine holes on a freshly-built `building` Path** (a thin/novel topic that didn't reach `spine_ready` synchronously); (3) handle the **spine-hole regression** (a hard-deprecated resource leaving a spine concept with no candidate → re-thicken). (2) and (3) are the same operation — targeted per-concept discovery (reuse `web-fallback.ts`) → validate/upsert/embed → re-attach + re-judge (reuse 2.5d-2) → recompute readiness — differing only in trigger. **Hole legitimacy (2.5d follow-up — evidence-based, deferred here):** before sourcing, classify each hole from its candidate coverage evidence. A _genuine gap_ (no resource teaches the concept; e.g. ML `unsupervised-learning-fundamentals`) is sourced via `web-fallback` as above. But a _conflation hole_ — an over-coarse concept where several `teaches@~0.4` candidates each cover a different slice and none clears the floor (the linear-algebra "Linear Independence, Basis, and Dimension" case) — is remediated by **SPLITTING the concept into finer nodes + re-attaching**, NOT by sourcing. The synchronous builder's semantic reviewer (2.5d `granularity` finding + author rule) catches most conflations at authoring time, but it is title-based and not guaranteed (a plausible-looking bundle can slip past); this post-attachment, coverage-evidence audit is the reliable backstop, and it lives here because the split-vs-source action is 2.5f's. **Remediation flow:** the request path (2.5g) calls `ensurePathMap`; if it returns `building`, it enqueues **one** remediation job for the Path and tells the user their course is being built. The worker single-flights per Path, fills holes, and on success flips `spine_ready` → builds the Track(s). **Exhaustion (per the locked decision): relax the bar first** (accept the best `uses`/below-floor `teaches` so the learner still gets a course), **then if a concept is still uncoverable, escalate to the human/agent review queue + page a developer.** Portable job infra (Postgres job table + worker; no Vercel Cron — see AGENTS.md) decided at block start. The "course ready" **email is deferred to Phase 3** (needs `User.email` from auth); until then the worker records readiness/escalation, no user email sent.
  - **Shipped** (PRs #85–#91, #93): `RemediationJob` schema + single-flight claim (partial unique index); `sourceForConcept` targeted per-concept discovery; evidence-based gap-vs-conflation classifier + relaxed-readiness honoring; the `remediatePath` orchestrator with promote-on-attach; `splitConcept` (conflation → finer nodes, conservative DAG inheritance) wired into a bounded iterative loop (finer nodes re-enter remediation); real `thickenSpine` with mastery-targeted discovery; **`addFrontierConcept` — on-demand frontier-concept enrichment** (responsibility (1): a learner's free-text request for a specialized concept not on the map is dedup/relevance-gated, authored as a frontier node wired into the DAG, and resourced); CLIs `scripts/remediate.ts` and `scripts/add-frontier.ts`. Job infra decision: **Postgres job table + manual drivers** now; auto-poller + request-path enqueue are 2.5g. Verified live — the `machine-learning` Path reached `spine_ready`, and a `reinforcement-learning` frontier node was added on request.
  - **Carried into 2.5g:** request-path enqueue + auto-poller (the automatic trigger) + admin endpoint (a synchronous HTTP handler would exceed the function timeout); stale-claim reclaim by age; subscriber model + "course ready" email (Phase 3); fold pre-existing unlinked `pending_review` rows into the re-judge set (today only freshly-sourced `insertedIds` are judged — harmless for a fresh `building` Path, matters for repeat/failed runs).
- [x] **2.5g — Cutover.** ✅ **Shipped as a stacked PR chain (#95–#102, merge bottom-up).** Repointed the create flow off the old `PathItem` curriculum agent onto the concept-map/Track pipeline, retired the dead subsystem, and dropped `PathItem`.
  - [x] **2.5g-1 ([#95](https://github.com/mwin02/learning-app/pull/95))** — `CourseRequest` model + claim primitives (`enqueue` / `claimNextQueued` via `FOR UPDATE SKIP LOCKED` / `finish` / `reclaimStale`). The durable queue between the route and the worker; carries the per-learner Track inputs + is the Phase-3 "course ready" notification hook. Deliberately **not** single-flighted (dedup lives one layer down in `ensurePathMap`'s advisory lock + the `RemediationJob` index).
  - [x] **2.5g-2 ([#96](https://github.com/mwin02/learning-app/pull/96))** — `ensurePathMap` reclaim of non-self-healing Paths: a `failed` Path, or a stale empty `building` Path (zero concepts + age-gated), is reset and rebuilt instead of "exists, skip"; a holey `building` Path stays for remediation.
  - [x] **2.5g-3 ([#97](https://github.com/mwin02/learning-app/pull/97))** — the out-of-band worker (`scripts/course-worker.ts`): `claim → ensurePathMap → remediate → buildTrack → log → finish`; `--watch`/`--once`, concurrency 1, no Vercel Cron (production trigger deferred to Cloud Run). Added `RemediationJob` stale-claim reclaim by age (the automatic counterpart to `--force`).
  - [x] **2.5g-4 ([#98](https://github.com/mwin02/learning-app/pull/98))** — **fire-and-forget route.** `POST /api/generate-path` validates + topic-gates + enqueues a `CourseRequest` → `202 { requestId }`; it no longer builds inline. Schema realigned to the Track-builder inputs (`difficulty` → optional `targetMastery`; added `goal`). Deleted the now-dead `path-service.ts`.
  - [x] **2.5g-5 ([#99](https://github.com/mwin02/learning-app/pull/99))** — reject pipeline repointed to **Path-side candidate-deprecation**: delete the `ConceptResource` links + `recomputeReadiness` (a hard reject reopening a spine hole regresses the Path `spine_ready → building`; the worker refills it). Replaced the old `PathItem active→removed` flip.
  - [x] **2.5g-5b ([#100](https://github.com/mwin02/learning-app/pull/100))** — broken-tracks triage page (`/playground/broken-tracks`), filter by soft/hard. (Beyond the original plan — see the design reversal below.)
  - [x] **2.5g-6 ([#101](https://github.com/mwin02/learning-app/pull/101))** — retired the old retrieval subsystem (`curriculum-retrieval.ts`, `curriculum-agent.ts`, `curriculum-critic.ts`) + `runWebFallback`'s topic-level entry (kept `sourceForConcept` + the shared engine) + the dead config (`FALLBACK_THRESHOLD`, `FALLBACK_TARGET_COUNT`, `PENDING_REVIEW_GATE_PER_TOPIC`, and the two orphaned `FALLBACK_*_DISCOVERY_*`/`_ITERATIONS` knobs). −1045 LOC.
  - [x] **2.5g-7 ([#102](https://github.com/mwin02/learning-app/pull/102))** — retired `PathItem` + `PathItemStatus` + the Path user columns (`title`/`summary`/`difficulty`/`input*`/`createdById`). `Path` is now `{ topic, status }` + concept-map relations. Migration hand-edited to preserve `Resource_embedding_idx`. Blast radius was empty (0 PathItem rows).
  - **Design reversal during the build — live-Track reach dropped.** The original scope had a hard reject _reach into live Tracks_ (promote a frozen alternate / flag the `LessonResource`). Reversed: **Tracks are immutable snapshots of a Path** (recorded in `schema.prisma` under `Track`). Only the _Path_ is kept accurate so future Tracks are valid; a built Track may keep pointing at a since-deprecated Resource (the row still exists). Broken/stale Tracks are triaged **manually** via the 2.5g-5b page rather than auto-healed. This removed the `LessonResource` `flagged`-state schema work the original scope implied.
  - **Carried-from-2.5f items, all landed:** request-path enqueue + auto-poller (the worker), stale-claim reclaim by age (both `CourseRequest` and `RemediationJob`), and the `CourseRequest` subscriber/dedupe model. _Still deferred:_ the "course ready" email (Phase 3 — needs `User.email`; the worker logs the Track to stdout as the stand-in), and folding pre-existing unlinked `pending_review` rows into the re-judge set (the planned **g-3b** — not yet built; see open items).
