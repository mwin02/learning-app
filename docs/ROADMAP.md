...
**Shipped:** Phases 1, 2, the full **2.5-AR curriculum-agent redesign**, the **topic partition vs. semantic search** redesign (Blocks 1/2b/2a/3, PRs #44–#47), the `pending_review → active` **promotion pipeline**, the **Phase 2.5 concept-map / Track-traversal** rebuild through the **2.5g cutover** (Path = per-topic concept map, Track = immutable learner traversal; the async worker builds Tracks off a durable `CourseRequest` queue), the **2.5h source-quality overhaul** (curated sourcing ladder + a real `trustScore`, [#128](https://github.com/mwin02/learning-app/pull/128)), and the **Phase 2.6 course player** (`/learn/[trackId]`, PRs #104–#111) — a read-only Track delivery UI with a centralized design-token system + OS-preference dark mode. The playground generates real sequenced Tracks end-to-end, and built Tracks render as a structured, dark-mode-aware course with embed/new-tab resources and anonymous progress.
...
### ⭐ NEXT UP — Phase 2.5h follow-up (block 2g) → Content agent (2.5i)

**Immediate (2g):** the 2.5h source-quality overhaul shipped ([#128](https://github.com/mwin02/learning-app/pull/128)); its follow-up is **generated on-ramp + scope-aware ranking** — the intro/on-ramp concept sources poorly under atomic curation, so we generate it (on-ramp only) and add a duration/scope bias for all concepts. Detail in the [block sequence](#block-sequence-each-300-loc-one-pr-per-block).

**Then 2.5i — Content agent (exercises):** `lib/agents/content/` generates `Exercise` records (text/MCQ first) for gap-prone Lessons, fanning out in parallel during track building under a per-build cost ceiling. It's the first content a learner can _act on_ rather than just read — and the missing half of the 2.6 lesson view (the exercise slot is already scaffolded in `LessonView`, and the playground/landing-page exit criteria both call for revealing exercises).

**Then:** **2.5j–l** (delivery-mode classifier → notebooks → playground map/Track) → **2.6a** (landing page) → 2.75 (Programs) → 3 (auth + Stripe) → 4–6.
...
- [ ] **2.5i — Content agent: exercises.** `lib/agents/content/` generates `Exercise` records (text/MCQ first) for gap-prone Lessons; fans out in parallel during track building. Cost ceiling per build.
- [ ] **2.5j — Delivery-mode classifier.** Per `LessonResource`, set `deliveryMode`. Embed allowlist + runtime header probe (`X-Frame-Options` / `CSP: frame-ancestors`) cached on `Resource`.
