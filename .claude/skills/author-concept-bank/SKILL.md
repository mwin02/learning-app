...
- **Know which DB that server was started against, and say so before authoring anything.** The probe URL is identical whether the server points at local Docker Postgres or at the production library, so nothing in the command says whose question banks you are about to rewrite. The server logs it on its first DB request as `{"event":"db.client_created","target":"host:port/dbname"}` — `localhost:55432/learning_app` is local and disposable, `…pooler.supabase.com:6543/postgres` is production and every bank edit is real. Ask the user to read that line off the dev-server terminal, and state the target in your first message. Pointed at the wrong DB the weak-bank worklist comes back **empty**, not failing — "no weak banks" is the symptom. See `docs/operator-tooling.md`.
